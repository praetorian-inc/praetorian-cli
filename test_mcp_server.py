...
def test_mcp_server_discovery():
    ...
    try:
        ...
        print(f"Discovered {len(server._discovered_tools)} tools:")
        for tool_name in sorted(server._discovered_tools.keys()):
            tool_info = server._discovered_tools[tool_name]
            print(f"  - {tool_name}: {tool_info['doc'].split(chr(10))[0] if tool_info['doc'] else 'No description'}")
        
        print("\nTesting recursion prevention...")
        mcp_tools = [name for name in server._discovered_tools.keys() if 'start_mcp_server' in name]
        if mcp_tools:
        ...
        print(f"Filtered server has {len(filtered_server._discovered_tools)} tools:")
        for tool_name in sorted(filtered_server._discovered_tools.keys()):
            print(f"  - {tool_name}")
        
        expected_tools = {'assets.add', 'risks.list'}
        actual_tools = set(filtered_server._discovered_tools.keys())
        ...
        if 'assets.add' in server._discovered_tools:
            tool_info = server._discovered_tools['assets.add']
