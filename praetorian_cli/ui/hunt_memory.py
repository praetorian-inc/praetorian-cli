...
from praetorian_cli.sdk.entities.hunts import (
    HUNT_SUMMARY_LOG_TITLE,
    hunt_memory_title_error,
)
from praetorian_cli.ui.hunt_data import build_hunt_memory
from praetorian_cli.ui.hunt_format import safe_multiline as _safe_multiline
...
class HuntMemoryBrowser:
    ...
    def load_current(self, force=False):
        ...
        if title is None:
            self.content = ''
            return True
        if not force and title in self.content_cache:
            self.content = self.content_cache[title]
            self.status = ''
            return True
        try:
        ...
        self.content_cache[title] = self.content
        self.status = ''
        return True
    ...
    def refresh(self):
        ...
        loaded = self.load_current(force=True)
        if loaded:
    ...
    def save(self):
        ...
        self.content_cache[title] = self.draft
    ...
    def _delete_current(self):
        ...
        self.content_cache.pop(title, None)
        self._replace_items(
...
def browse_hunt_memory(console, hunts, hunt_id, *, start_new=False):
    """Browse Hunt memory in a TTY, optionally opening the new-item form."""
    if not supports_fullscreen_memory_browser():
        items, _ = hunts.list_memory(hunt_id)
        console.print(build_hunt_memory(items))
        return False

    with console.status('Loading Hunt memory…', spinner='dots'):
...
def build_hunt_memory_preview(browser, *, embedded=False):
    """Render the selected item's full multiline content."""
    title = browser.current_title
    if title is None:
        body = Text(
            'Press n to create the first memory item.',
            style='dim',
        )
        ...
        shown_title = title
    subtitle = (
        'Read-only preview · Enter/m opens memory editor'
        if embedded
        else 'Read-only preview · Enter/e to edit'
    )
    return Panel(
        body,
...
def build_hunt_memory_browser(browser, *, embedded=False):
    """Build a deterministic vertical snapshot for rendering tests."""
    return Group(
        build_hunt_memory_navigation(browser),
        Text(''),
        build_hunt_memory_preview(browser, embedded=embedded),
...
def _run_memory_browser(console, browser):
    ...
    initial_focus = (
        title_window if browser.mode == 'title' else navigation_window
    )
    layout = Layout(
        HSplit([header, body, footer]),
        focused_element=initial_focus,
    )
...
def _title_error(title):
    return hunt_memory_title_error(title)
