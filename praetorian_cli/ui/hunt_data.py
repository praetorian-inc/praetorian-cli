...
def _matches_severity(status_code, status_label, severity):
    ...
    return status_code[1:2] == code or status_label.endswith(severity)
...
def _severity(risk):
    ...
    for severity in SEVERITY_CODES:
        if label.endswith(severity):
            return severity
    # Guard stores severity in the second character (for example TC, OH,
    # and deleted DCF where the trailing character is the deletion reason).
    code = str(risk.get('status') or '')[1:2].upper()
    return next(
