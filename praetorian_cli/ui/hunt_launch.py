...
def supported_hunt_credentials(records):
    """Return selectable run-scoped credentials with stable identifiers."""
    return [
        credential for credential in records or []
        if isinstance(credential, dict)
        and _credential_id(credential)
...
class HuntLaunchConfigurator:
    ...
    def __init__(self, config, targets, endpoint='', credentials=None):
        ...
        self.credentials = supported_hunt_credentials(credentials)
    ...
    def confirmed_config(self):
        ...
        config['_resources_changed'] = self.resources_changed
    ...
    def _credential_options(self, credential_type):
        ...
        if matching:
            none_label = f'None ({len(matching)} available)'
            none_description = (
                'No credential selected. Press Left/Right, Space, or Enter '
                'to choose an available credential.'
            )
        else:
            none_label = 'Unavailable'
            none_description = (
                f'No {credential_type} credentials were returned for this '
                'account. Add or authorize one, then reopen the launcher.'
            )
        options = [(None, none_label, none_description)]
