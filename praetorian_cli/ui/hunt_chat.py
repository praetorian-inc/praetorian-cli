...
def watch_pending_hunt_interactions(
    sdk,
    hunt_id,
    pending,
    console,
    *,
    ...
    """Watch for new Hunt interactions using one shared polling contract."""
    handled = set()
    seen = {hunt_interaction_key(row) for row in pending}
    review(pending, handled)
    try:
        while True:
            time.sleep(interval)
            try:
                current = sdk.hunts.list_interactions(
                    hunt_id,
                    status='pending',
                )
            except Exception:
            ...
            review(new_interactions, handled)
            seen.update(
                hunt_interaction_key(row) for row in new_interactions
            )
    except KeyboardInterrupt:
        notify('Stopped watching Hunt interactions.', 'info')
    return handled
