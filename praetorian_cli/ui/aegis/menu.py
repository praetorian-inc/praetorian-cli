...
class AegisMenu:
    ...
    def handle_help(self, args):
        ...
        if args and args[0] in ['ssh', 'list', 'info', 'job', 'set']:
            ...
            self.console.print("\n  Available commands:")
            self.console.print("    set <id>              - Select an agent by number or client_id")
            self.console.print("    list [--details]      - List agents with optional details")
            self.console.print("         [--filter <text>] - Filter agents by hostname/OS")
            self.console.print("    ssh [options]         - SSH to selected agent")
            self.console.print("    info                  - Show selected agent information")
            self.console.print("    job [capabilities]    - Run job on selected agent")
            self.console.print("    reload                - Refresh agent list")
            self.console.print("    help [command]        - Show this help or command-specific help")
            self.console.print("    quit/exit             - Exit the interface")
            self.console.print("\n  Examples:")
            self.console.print(f"    [{self.colors['dim']}]set 1                   # Select agent by number[/{self.colors['dim']}]")
            self.console.print(f"    [{self.colors['dim']}]set abc123              # Select agent by client_id[/{self.colors['dim']}]")
            self.console.print(f"    [{self.colors['dim']}]ssh -D 1080             # SSH with SOCKS proxy on port 1080[/{self.colors['dim']}]")
            self.console.print(f"    [{self.colors['dim']}]list --filter windows   # Show only Windows agents[/{self.colors['dim']}]")
            self.console.print()
            self.pause()
