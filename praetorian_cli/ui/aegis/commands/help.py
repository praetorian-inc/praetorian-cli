...
def handle_help(menu, args):
    ...
    commands_table.add_row("ssh [options]", "SSH to selected agent (splits tmux pane when available)")
    commands_table.add_row("ssh --window", "SSH in a new tmux window instead of a split pane")
    commands_table.add_row("ssh --block", "SSH in blocking mode (takes over terminal)")
    commands_table.add_row("ssh list", "Show active SSH panes")
    commands_table.add_row("ssh kill <id|all>", "Kill an SSH pane by number or all")
    ...
    examples_table.add_row("ssh --window", "SSH in a separate tmux window")
    examples_table.add_row("ssh --block", "Blocking SSH (old behavior)")
    examples_table.add_row("ssh list", "Show active SSH panes")
    examples_table.add_row("ssh kill 1", "Kill first SSH pane")
    examples_table.add_row("ssh kill all", "Kill all SSH panes")
