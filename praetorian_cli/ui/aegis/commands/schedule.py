...
def view_schedule(menu, args):
    ...
    try:
        ...
        menu.console.print()
        menu.console.print(f"  [bold]Schedule Details[/bold]")
        menu.console.print()
        ...
        menu.console.print()
        menu.console.print(f"  [bold]Schedule[/bold]")
        ...
        menu.console.print()
        menu.console.print(f"  [bold]Dates[/bold]")
        ...
        menu.console.print()
        menu.console.print(f"  [bold]Execution[/bold]")
        next_exec = schedule.get('nextExecution', '')
        ...
        if config:
            menu.console.print()
            menu.console.print(f"  [bold]Configuration[/bold]")
...
def add_schedule(menu):
    ...
    colors = getattr(menu, 'colors', DEFAULT_COLORS)
    menu.console.print("\n  [bold]Create New Schedule[/bold]\n")

    # Select capability
    capability = _interactive_capability_picker(menu)
    if not capability:
        return
    ...
    # Optional parameters
    custom_params = _configure_parameters(menu, capability_info, has_credential=bool(config.get('credential_id')))
    if custom_params:
...
def edit_schedule(menu, args):
    ...
    try:
        menu.console.print(f"\n  [bold]Edit Schedule: {schedule_id[:10]}[/bold]")
        menu.console.print(f"  Capability: {schedule.get('capabilityName', 'N/A')}")
        menu.console.print()
        ...
        menu.console.print(f"\n[green]✓ Schedule updated successfully[/green]")
        menu.console.print(f"  Next execution: {format_datetime(result.get('nextExecution', ''))}")
...
# --- Delegated helpers from job.py ---

def _interactive_capability_picker(menu):
    """Interactive capability picker (reused from job.py)."""
    from .job import _interactive_capability_picker as picker
    return picker(menu, None)


def _select_domain(menu):
    """Interactive domain selection (reused from job.py)."""
    from .job import _select_domain as selector
    return selector(menu)


def _select_credentials(menu):
    """Interactive credential selection (reused from job.py)."""
    from .job import _select_credentials as selector
    return selector(menu)


def _configure_parameters(menu, capability_info, has_credential=False):
    """Interactive parameter configuration (reused from job.py)."""
    from .job import _configure_parameters as configurator
    return configurator(menu, capability_info, has_credential)
