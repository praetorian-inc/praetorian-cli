...
def _load_skill_files(paths):
    ...
    Returns a tuple of (formatted content, list of warning strings) so
    callers can surface credential-filename warnings in the chat UI.
    """
    parts = []
    warnings = []
    for path in paths:
...
class ConversationApp(App):
    ...
    def call_conversation_api(self, message: str) -> Dict:
        ...
        if self._skills:
            skill_context, warnings = _load_skill_files(self._skills)
