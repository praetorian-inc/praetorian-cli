...
from praetorian_cli.ui.hunt_format import (
    safe_text as _safe,
    timestamp_sort_key as _timestamp_sort_key,
...
class WorkflowBrowser:
    ...
    def step_indices(self):
        return list(range(len(_workflow_steps(self.current))))
    ...
    def current_step_index(self):
        indices = (
            self.step_indices
            if self.step_mode
            else self.conversation_step_indices
        )
        if not indices:
            return None
    ...
    def move_step(self, delta):
        """Move through workflow children, or chat steps in legacy mode."""
        if not self.step_mode:
            return self.move_chat_step(delta)
        indices = self.step_indices
        if not indices:
            self.notice = 'This workflow has no steps.'
            return False
        selected = self.current_step_index
        position = indices.index(selected)
        next_position = min(max(position + delta, 0), len(indices) - 1)
        if next_position == position:
            return False
        self.step_cursors[self.cursor] = indices[next_position]
        self.expand()
        self.notice = ''
        return True

    def move_chat_step(self, delta):
        """Cycle through chat-enabled steps for the standalone browser."""
        ...
        if selected in indices:
            position = (indices.index(selected) + delta) % len(indices)
        else:
            position = 0 if delta > 0 else len(indices) - 1
        self.step_cursors[self.cursor] = indices[position]
        self.expand()
        self.notice = ''
        return True

    def enter(self):
        """Enter step selection, then return the selected conversation ID."""
        if not self.step_mode:
            ...
            self.expand()
            self.step_mode = True
            self.step_cursors[self.cursor] = self.step_indices[0]
            self.notice = 'Select a step with Up/Down, then press Enter.'
            return None
        conversation_id = self.current_conversation_id
        if not conversation_id:
            self.notice = 'Selected step has no live conversation.'
            return None
        return conversation_id

    def leave_steps(self):
        if not self.step_mode:
            return False
        self.step_mode = False
        self.notice = ''
...
def build_hunt_workflow_browser(browser):
    """Render the unified-view workflow cursor, expansion, and chat step."""
    if not browser.runs:
    ...
    detail = (
        _iteration_panel(
            ...
            browser.iteration_number(),
            selected_step_index=(
                browser.current_step_index if browser.step_mode else None
            ),
            interactive=browser.step_mode,
        )
        if browser.current_is_expanded
        else Text('Press Space to expand the highlighted workflow.', style='dim')
    )
    renderables = [_workflow_navigation_table(browser), detail]
    if browser.notice:
        renderables.append(Text(browser.notice, style='yellow'))
