...
def _select_entity_keys_with_prompts(
    ...
    """Drive the shared selector state with line-oriented prompts."""
    selector = EntitySelector(
        entities,
        search_entities=search_entities,
        next_offset=next_offset,
    ...
    while True:
        visible = selector.visible_rows
        console.print(_selection_table(
            visible,
            title,
            set(selector.selected_keys),
        ))
        if selector.status:
            console.print(Text(f'  {selector.status}', style='dim'))

        visible_end = min(
            selector.visible_start + len(visible),
            len(selector.rows),
        )
        if len(selector.rows) > MAX_VISIBLE_ENTITIES:
            console.print(
                f'  Showing {selector.visible_start + 1}-{visible_end} of '
                f'{len(selector.rows)} results on this server page.',
                style='dim',
            )

        _, next_offset = selector.pages[selector.page_index]
        has_previous = selector.visible_start > 0 or selector.page_index > 0
        has_next = (
            selector.visible_start + MAX_VISIBLE_ENTITIES < len(selector.rows)
            or selector.page_index + 1 < len(selector.pages)
        ...
        if lowered.startswith('/search'):
            selector.begin_search()
            selector.query = response[7:].strip()
            selector.apply_search()
            continue
        if lowered in {'next', 'n'}:
            selector.next_chunk()
            continue
        if lowered in {'prev', 'previous', 'p'}:
            selector.previous_chunk()
            continue
        try:
        ...
        for index in indices:
            selector.cursor = selector.visible_start + index
            selector.toggle_current()
