...
def ask(sdk, message, mode, new_conversation, output_format, skill):
    ...
    # os.makedirs' mode is only applied to newly-created directories, and
    # os.open's mode is only applied when the file is newly created, so an
    # already-existing directory/file would otherwise keep looser permissions.
