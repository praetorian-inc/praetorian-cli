...
def _sdk(candidates=None, credentials=None, credential_error=None):
    return SimpleNamespace(
        hunts=FakeHunts(),
        assets=FakeAssets(candidates),
        aegis=FakeAegis(),
        credentials=FakeCredentials(credentials, credential_error),
    )
...
def test_wizard_preloads_credentials_before_switching_to_internal(monkeypatch):
    credential_id = 'ad-credential-1'
    sdk = _sdk(credentials=[{
    ...
    def configure(_console, config, _targets, _endpoint, credentials):
        captured['credentials'] = list(credentials)
        return ({
            **config,
            'surface': 'internal',
            'scope_mode': 'specific',
            'scope': ['#asset#internal.example#10.0.0.5'],
            'prompt': DEFAULT_HUNT_MANDATES['internal'],
    ...
    result = CliRunner().invoke(
        hunt,
        ['launch', '--endpoint', ENDPOINT_ID],
        obj=sdk,
        input='y\n',
    )

    assert result.exit_code == 0, result.output
    assert captured['credentials'][0]['credentialId'] == credential_id
    payload = sdk.hunts.create_calls[0]
    assert payload['endpoint_required'] is True
    assert payload['endpoint_id'] == ENDPOINT_ID
    assert payload['credential_ids'] == [credential_id]
...
def test_wizard_reports_credential_api_failure_without_blocking_internal_hunt(
    monkeypatch,
):
    sdk = _sdk(credential_error=RuntimeError('broker unavailable'))
    monkeypatch.setattr(
        'praetorian_cli.handlers.hunt.supports_fullscreen_wizard',
        lambda: True,
    )
    ...
    assert result.exit_code == 0, result.output
    assert 'Credential options unavailable: broker unavailable' in result.output
    assert sdk.hunts.create_calls[0]['endpoint_required'] is True
    assert sdk.hunts.create_calls[0]['credential_ids'] is None


def test_external_wizard_can_launch_when_optional_credentials_are_unavailable(
    monkeypatch,
):
    sdk = _sdk(credential_error=RuntimeError('broker unavailable'))
    monkeypatch.setattr(
    ...
    assert result.exit_code == 0, result.output
    assert sdk.hunts.create_calls[0]['endpoint_required'] is False


def test_wizard_surface_round_trip_revalidates_endpoint_resources(monkeypatch):
    sdk = _sdk()
    monkeypatch.setattr(
        'praetorian_cli.handlers.hunt.supports_fullscreen_wizard',
        lambda: True,
    ...
    assert result.exit_code == 0, result.output
    assert sdk.aegis.calls == 2
    assert sdk.hunts.create_calls[0]['endpoint_id'] == ENDPOINT_ID
