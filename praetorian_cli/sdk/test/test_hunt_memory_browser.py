...
def test_memory_browser_reuses_visited_item_content_until_refresh():
    ...
    get_calls = [call for call in hunts.calls if call[0] == 'get']
    assert get_calls == [
        ('get', 'hunt-1', 'alpha.md'),
        ('get', 'hunt-1', 'beta.md'),
    ]

    hunts.contents['beta.md'] = 'fresh beta context'
    browser.refresh()
    assert browser.content == 'fresh beta context'
...
def test_fullscreen_new_item_mode_accepts_title_content_and_save():
    from prompt_toolkit.application import create_app_session
    from prompt_toolkit.input.defaults import create_pipe_input
    from prompt_toolkit.output import DummyOutput

    hunts = FakeHunts()
    browser = HuntMemoryBrowser(hunts, 'hunt-1')
    assert browser.request_new() is True

    with create_pipe_input() as input_pipe:
        input_pipe.send_text('operator-notes.md\rfirst note\x13q')
        with create_app_session(input=input_pipe, output=DummyOutput()):
            hunt_memory._run_memory_browser(Console(), browser)

    assert ('save', 'hunt-1', 'operator-notes.md', 'first note') in hunts.calls


def test_tty_memory_browser_can_open_directly_in_new_item_mode(monkeypatch):
    hunts = FakeHunts()
    opened = []
    monkeypatch.setattr(
        hunt_memory,
    ...
    monkeypatch.setattr(
        hunt_memory,
        '_run_memory_browser',
        lambda _console, browser: opened.append(browser.mode),
    )

    assert browse_hunt_memory(
        Console(),
        hunts,
        'hunt-1',
        start_new=True,
