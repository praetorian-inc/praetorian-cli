...
class TestZCli:
    ...
    def test_seed_cli(self):
        ...
        self.verify(f'add seed -v {o.seed_dns} -t domain')

        self.verify('list seeds -p all', [o.seed_dns_key])
        self.verify('list seeds -t domain -p all', [o.seed_dns_key])
        self.verify(f'list seeds -t domain -f "{o.seed_dns}"', [o.seed_dns_key])
        self.verify(f'list seeds -t domain -f "{o.seed_dns}" -p first', [o.seed_dns_key])
        self.verify(f'list seeds -t domain -f "{o.seed_dns}" -p all', [o.seed_dns_key])
        self.verify(f'list seeds -t domain -f "{o.seed_dns}" -p first', [o.seed_dns_key])
        ...
        self.verify(f'get seed "{o.seed_dns_key}"',
                    [o.seed_dns_key, f'"status": "{Seed.PENDING.value}"'])

        self.verify(f'update seed -s {Seed.ACTIVE.value} "{o.seed_dns_key}"')
        self.verify(f'get seed "{o.seed_dns_key}"',
                    [o.seed_dns_key, f'"status": "{Seed.ACTIVE.value}"'])

        self.verify(f'delete seed "{o.seed_dns_key}"')
        self.verify(f'get seed "{o.seed_dns_key}"', [f'"status": "{Seed.DELETED.value}"'])
