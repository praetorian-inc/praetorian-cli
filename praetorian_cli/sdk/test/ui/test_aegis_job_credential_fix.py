...
def test_credential_uuid_passed_not_fetched(monkeypatch):
    ...
    responses = {
        'capabilities': {
            'ad-enum': {
                ...
                'target': 'addomain',
                'parameters': []  # No parameters to avoid config embedding
            }
        },
        'credentials': [
            {
    ...
    confirm_responses = [True, True, True]  # Use suggested cap, attach credentials, run job
    confirm_index = [0]
    ...
    monkeypatch.setattr('praetorian_cli.ui.aegis.commands.job._select_credentials',
                        lambda m: '#credential#env-integration#active-directory#test-uuid-1234')
...
def test_multiple_credentials_passed_as_uuids(monkeypatch):
    ...
    monkeypatch.setattr('praetorian_cli.ui.aegis.commands.job._select_domain', lambda m: 'example.local')
    monkeypatch.setattr('praetorian_cli.ui.aegis.commands.job._select_credentials',
                        lambda m: '#credential#env#active-directory#uuid-1')
