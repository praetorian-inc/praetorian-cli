...
class Hunts:
    ...
    def _memory_path(self, hunt_id, title):
        ...
        if not hunt_id:
            raise ValueError('hunt ID is required')
        if not re.fullmatch(r'[a-zA-Z0-9._\- ]{1,128}', title):
            raise ValueError(
                'memory title must use 1-128 letters, numbers, spaces, dots, underscores, or hyphens'
            )
    ...
    def _memory_item_api_path(self, hunt_id, title):
        ...
        if normalized_title.lower() == 'summary.log':
